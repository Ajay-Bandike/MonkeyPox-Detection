...
def prediction(request):
    ...
    if request.method == 'POST':
        ...
        f11 = float(request.POST.get('Close_Contact', 0))
        ...
        import pandas as pd
        feature_names = ['COVID_Symptoms', 'COVID_Tests', 'COVID_Vaccination', 'Age',
                 'Contact_with_Monkeypox', 'Travel_History', 'Healthcare_Worker',
                 'Body_Temperature', 'Hand_Hygiene', 'Mask_Usage', 'Close_Contact']
        PRED = pd.DataFrame([[f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11]], columns=feature_names)
        xgp = model.predict(PRED)[0]

        if xgp == 0:
            message_text = "The Person is not affected with MONKEY_POX"
            message_color = "green"
        else:
            message_text = "The Person is affected with MONKEY_POX"
            message_color = "red"
    ...
    # Handle GET request with no message
    return render(request, 'prediction.html', {
        'msg_text': None,
        'msg_color': None
    })
